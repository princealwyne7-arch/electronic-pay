...
app.get('/api/status', (req, res) => {
    ...
    // AI Score responds to live traffic nodes
    const aiScore = Math.min(999, 820 + (successfulTxs.length * 14));
    res.json({ transactions, todayTotal, aiScore, latency: Math.floor(Math.random() * 28) + 12 });
});
...
<head>
    ...
    <style>
        ...
        .intel-ticker span { display: inline-block; animation: scroll 18s linear infinite; }
...
<body>
    ...
    <div id="assetOverlay">
        ...
        <div class="card" style="background:var(--primary); color:white; margin-top:20px;">
            ...
            <div style="font-size:10px; color:var(--accent)">● AI Market Active</div>
        </div>
        <div class="a-grid">
    ...
    <div id="tab-insights" class="tab-content">
        <div class="intel-ticker">
            <span>BTC: $95,210.40 | ETH: $2,550.12 | KES/USD: 129.50 | AI ENGINE: ACTIVE | NODE STATUS: 100% SECURE</span>
        </div>
        <div class="card" style="border-left: 5px solid var(--accent);">
            ...
            <div style="margin-top:20px;">
                <div style="display:flex; justify-content:space-between; font-size:11px; padding:8px 0; border-bottom:1px solid #f1f5f9;">
                    <b>Fraud Probability</b><span style="color:var(--accent)">0.001%</span>
                </div>
                <div style="display:flex; justify-content:space-between; font-size:11px; padding:8px 0; border-bottom:1px solid #f1f5f9;">
                    <b>Node Prediction</b><span id="predictVal" style="color:var(--accent)">KES 0</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                document.getElementById('predictVal').innerText = 'KES ' + Math.floor(data.todayTotal * 1.15).toLocaleString();
                ...
                bar.style.height = (data.latency * 2.2) + 'px';
                ...
                document.getElementById('activityFeed').innerHTML = data.transactions.map(t => \`
                    ...
                    </div>\`).join('') || 'Waiting for node activity...';
            } catch(e) {}
        }
