...
// PAYNECTA LOGIC: Remains original
const translateNecta = (body) => {
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                .card { background: #111; border: 1px solid #00f2ff; padding: 25px; border-radius: 20px; width: 100%; max-width: 400px; text-align: center; box-shadow: 0 0 15px #00f2ff55; }
                ...
                .btn { width: 100%; padding: 16px; border: none; border-radius: 10px; font-weight: bold; cursor: pointer; margin-top: 10px; text-transform: uppercase; }
                .btn-necta { background: #00ff88; color: #000; }
                .btn-stack { background: #ffd700; color: #000; }
                .tx-list { width: 100%; max-width: 400px; margin-top: 25px; border: 1px solid #333; border-radius: 15px; background: #111; padding: 10px; box-sizing: border-box; }
        ...
        <body onclick="document.getElementById('beep').play().catch(() => {})">
            <div class="card">
                ...
                <div id="total" style="font-size:24px; margin:10px 0;">KES 0</div>
            ...
            <div class="tx-list" id="list">Waiting...</div>
            ...
            <script>
                async function update() {
                    ...
                    data.transactions.forEach(t => {
                        ...
                        html += '<div class="tx-row"><div><b>'+t.phone+'</b><br><small>'+t.provider+'</small></div><div style="text-align:right;"><b style="color:#ffd700;">KES '+t.amount+'</b><br><span style="color:'+(isSuccess ? '#00ff00' : '#ff4c4c')+'">'+t.status+'</span></div></div>';
                    });
    ...
    try {
        ...
        if (provider === 'paynecta') {
            const resp = await axios.post('https://paynecta.co.ke/api/v1/payment/initialize', {
                code: process.env.PAYMENT_CODE, mobile_number: cleanPhone, amount, email: "princealwyne7@gmail.com",
            ...
            // FIX: Sending +254 for Paystack to stop the 400 error
...
app.post('/callback', (req, res) => {
    ...
    let tx = transactions.find(t => (ref && String(t.id) === String(ref)) || bodyStr.includes(String(t.phone)));
    
    if (tx) {
        if (tx.provider === 'PAYSTACK') {
            // RAW MESSAGE DISPLAY: Only grabs the "message" part as requested
