...
// CALLBACK LOGIC (Detects Success/Cancel)
app.post('/callback', async (req, res) => {
    const data = JSON.stringify(req.body).toLowerCase();
    // Search for transaction by merchant ID or phone
    const filter = { $or: [
        { id: req.body.merchant_request_id },
        { phone: req.body.mobile_number }
    ]};
    
    let status = 'Failed ⚠️';
    if (data.includes('success') || data.includes('"0"') || data.includes('completed')) {
        status = 'Successful ✅';
    } else if (data.includes('cancel') || data.includes('1032')) {
        status = 'Cancelled ❌';
    }
...
<head>
    ...
    <title>Elite Pay | Command Center</title>
    <style>
        ...
        .pulse-dot { width: 8px; height: 8px; background: var(--accent); border-radius: 50%; animation: blink 1s infinite; display:inline-block; margin-right:5px;}
        ...
        input { width:100%; padding:14px; margin:8px 0; border:1px solid #e2e8f0; border-radius:12px; box-sizing:border-box; font-size:16px; }
        .btn-exec { width:100%; padding:16px; background: var(--accent); color:white; border:none; border-radius:12px; font-weight:800; cursor:pointer; }
...
<body>
    <div class="topbar">
        ...
        <div style="font-size:10px; font-weight:bold;"><div class="pulse-dot"></div><span id="latency">PULSE: 0ms</span></div>
    </div>

    <div id="tab-dash" class="tab-content active-tab">
        <div class="card balance-card">
            <small style="opacity:0.7">TOTAL REVENUE (SUCCESSFUL)</small>
            <h1 id="totalRev" style="margin:5px 0; font-size:32px;">KES 0</h1>
            <div style="font-size:11px; background:rgba(255,255,255,0.1); display:inline-block; padding:4px 10px; border-radius:8px;">Node: Active</div>
        </div>

        <div class="card">
            <h3 style="margin-top:0;">Admin STK Push</h3>
            <input type="password" id="adminPin" placeholder="Manager PIN">
            <input type="number" id="pPhone" placeholder="2547XXXXXXXX">
            <input type="number" id="pAmount" placeholder="Amount (KES)">
            <button onclick="runPush()" class="btn-exec">INITIALIZE TRANSACTION</button>
        </div>
    </div>

    <div id="tab-activity" class="tab-content">
        <div class="card">
            <h3 style="margin:0 0 15px 0;">Live Transaction Stream</h3>
            <div id="activityFeed">Connecting to Ledger...</div>
        </div>
    </div>

    <div id="tab-security" class="tab-content">
        <div class="card">
            <h3>🔊 Audio Engine (15+ Channels)</h3>
            <p style="font-size:11px; color:#64748b;">Select frequency to test system alerts.</p>
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">
                ${Array.from({length: 15}, (_, i) => \`<button class="fx-btn" onclick="playSfx(\${i+1})">FX \${i+1}</button>\`).join('')}
            </div>
            <button class="btn-exec" onclick="location.reload()" style="background:#64748b; margin-top:20px;">REBOOT SYSTEM</button>
        </div>
    </div>

    <nav class="bottom-nav">
        <div class="nav-item active" onclick="switchTab('dash', this)">🏠<br>Home</div>
    ...
    <script>
        let lastCount = 0;
        const playSfx = (idx) => {
            // Using placeholder logic for the 15+ requested sounds
        ...
        async function refresh() {
            try {
                ...
                if(data.transactions.length > lastCount) {
                    if(lastCount > 0) playSfx(1); // Play sound on new txn
                    lastCount = data.transactions.length;
                }

                document.getElementById('activityFeed').innerHTML = data.transactions.map(t => \`
                    <div style="display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid #f1f5f9; font-size:13px;">
                        <span><b>\${t.phone}</b><br><small style="color:#94a3b8;">\${t.time}</small></span>
                        <span style="text-align:right;"><b>KES \${t.amount}</b><br>
                        <small style="color:\${t.status.includes('Successful') ? '#28a745' : '#ef4444'}; font-weight:bold;">\${t.status}</small></span>
                    </div>\`).join('') || 'No Transactions Found';
...
app.listen(PORT, () => console.log(\`Elite Engine Live on Port \${PORT}\`));
