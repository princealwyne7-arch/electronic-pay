...
app.get('/api/status', (req, res) => {
    ...
    const totals = {
        ...
        yesterday: 0, // Simplified for now
        month: 0
    };
    ...
    res.send(`
        ...
        <body onclick="document.getElementById('successSound').play().then(p=>document.getElementById('successSound').pause())">
            ...
            <div class="history-card">
                <h4 style="margin:0 0 10px 0;">Live Status</h4>
            ...
            <script>
                async function updateStatus() {
                    ...
                    list.innerHTML = data.transactions.map(t => {
                        ...
                        return '<div class="tx-row"><div style="text-align:left;"><b>'+t.phone+'</b><br><small>'+t.time+'</small></div><div style="text-align:right;"><b>KES '+t.amount+'</b><br><span style="color:'+(isSuccess?'#28a745':t.status.includes('Processing')?'#f0ad4e':'#d9534f')+'">'+t.status+'</span></div></div>';
                    }).join('') || 'No activity';
                }
