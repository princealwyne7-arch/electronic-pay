...
// --- PAYNECTA STK PUSH LOGIC ENGINE ---
...
app.post('/api/stk-push', async (req, res) => {
    ...
    try {
        const response = await axios.post('https://api.paynecta.com/v1/stk/push', {
            ...
            headers: { 
                'Authorization': `Bearer ${PAYNECTA_SECRET}`,
                'Content-Type': 'application/json' 
            }
        });
        res.status(200).json({ status: "SUCCESS", message: "STK PROMPT SENT", ref: response.data.checkout_id });
    } catch (error) {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; outline: none; }
        ...
        /* BANKING MODAL OVERLAY - SCROLLABLE */
        #banking-overlay {
...
<body>
    <div class="app-shell">
        ...
        <main class="workspace">
            ...
            <div class="grid-meters">
                ...
                <div class="meter-card"><div class="meter-label">Transactions/Sec</div><div class="meter-value">38 TPS</div></div>
                ...
                <div class="ai-neural-card">
                    ...
                    <div>
                        <div class="ai-logic-stream">
                            <div class="logic-item" style="border-color:var(--neon-red)">Paynecta Engine: Live Secrets Detected.</div>
                            <div class="logic-item" style="border-color:var(--neon-blue)">STK Push Gateway Ready.</div>
                        </div>
        ...
        <div class="engine-modal">
            ...
            <div class="modal-body">
                <div class="bank-feature-row" style="background:rgba(57, 255, 20, 0.05); border-left:3px solid var(--neon-green);">
                    <div><div style="font-family:Orbitron; color:var(--neon-green);">STK PUSH</div><small>Paynecta Engine</small></div>
                    <div style="font-size:11px; color:#64748b;">Direct Society Trigger via Secret Keys</div>
                    <button class="btn-core" style="background:var(--neon-green); color:black; border:none;" onclick="triggerSTK()">EXECUTE</button>
                </div>
                <div class="bank-feature-row"><div><div style="font-family:Orbitron;">WIRE</div><small>RTGS</small></div><div>Global Node Tunnel</div><button class="btn-core" onclick="play(4)">OPEN</button></div>
                <div class="bank-feature-row"><div><div style="font-family:Orbitron;">VAULT</div><small>INTERNAL</small></div><div>Sync Nodes</div><button class="btn-core" onclick="play(5)">SYNC</button></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function triggerSTK() {
            play(11);
            const phone = prompt("Target Phone (e.g. 254...):");
...
app.listen(3000, () => console.log("System Engine V4 Pro Online - Paynecta STK Active"));
