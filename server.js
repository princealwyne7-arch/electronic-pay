...
app.post('/admin/push', async (req, res) => {
    ...
    // Live Activity: Wrong PIN Check
    if (pin !== "5566") return res.status(403).json({ error: "Access Denied", event: "WRONG_PIN" });
    const trackingId = `BNK-${Date.now()}`;
    
    // Persistent Save - Initializing Processing Activity
    ...
    try {
        ...
        res.json({ success: true, trackingId, event: "PROCESSING" });
    } catch (err) { 
        await Transaction.updateOne({ id: trackingId }, { status: "Failed ❌" });
        res.status(500).json({ success: false, event: "FAILED" });
    }
});
...
<body>
    ...
    <script>
        let isAdmin = false;
        let lastStatusMap = new Map(); // Tracker for Live Activity events
        ...
        async function update() {
            try {
                ...
                // --- LIVE ACTIVITY SOUND ENGINE ---
                if (data.transactions && data.transactions.length > 0) {
                    const latest = data.transactions[0];
                    if (lastStatusMap.get(latest.id) !== latest.status) {
                        // Respond to background events with specific SFX
                        if (latest.status.includes('Successful')) playSfx(1);
                        else if (latest.status.includes('Processing')) playSfx(12);
                        else if (latest.status.includes('Cancelled')) playSfx(4);
                        else if (latest.status.includes('Failed')) playSfx(3);
                        lastStatusMap.set(latest.id, latest.status);
                    }
                }
        ...
        async function runPush() { 
            ...
            const response = await fetch('/admin/push', { 
                method: 'POST', 
            ...
            // Handle Direct Live Activity Response
            if (result.event === "WRONG_PIN") { playSfx(10); alert("Access Denied: Wrong PIN"); }
            if (result.event === "PROCESSING") { playSfx(12); }
            if (result.event === "FAILED") { playSfx(3); }
