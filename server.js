...
<head>
    ...
    <style>
        :root { --primary: #0f172a; --accent: #28a745; --bg: #f8fafc; --card: #ffffff; --red: #ef4444; --ai-glow: #3b82f6; }
        ...
        .drawer-item { padding:15px; border-bottom:1px solid rgba(255,255,255,0.05); color:white; text-decoration:none; display:block; font-size:14px; cursor:pointer; }
        ...
        .intel-nav-item { position: relative; background: #f1f5f9; padding: 10px 18px; border-radius: 14px; font-size: 10px; font-weight: 800; white-space: nowrap; border: 1px solid #e2e8f0; color: #64748b; transition: all 0.3s; }
        .intel-nav-item.active { background: var(--primary); color: white; border-color: var(--primary); transform: scale(1.05); }
        ...
        #assetOverlay, #aiOverlay { position:fixed; bottom:-100%; left:0; width:100%; height:92%; background:white; border-radius:30px 30px 0 0; z-index:3500; transition:0.4s cubic-bezier(0.4, 0, 0.2, 1); padding:25px; box-sizing:border-box; overflow-y:auto; }
        ...
        .a-feat { background:#f8fafc; padding:15px; border-radius:18px; border:1px solid #f1f5f9; transition: 0.2s; }
        .a-feat:active { background: #e2e8f0; transform: scale(0.97); }
...
<body>
    ...
    <div id="aiOverlay">
        ...
        <div class="card" style="background:var(--primary); color:white; margin-top:20px; border-left: 5px solid var(--ai-glow);">
            <small style="opacity:0.7">GROWTH PREDICTION (24H)</small>
            <h2 id="aiPredictValue" style="margin:5px 0; color:var(--ai-glow);">KES --</h2>
            <div style="font-size:10px; color:#4ade80">● Neural Engine Active</div>
        </div>
        <div class="a-grid">
            <div class="a-feat" onclick="playSfx(1)">🚀<b>Growth Boost</b><small>Optimize Routes</small></div>
            <div class="a-feat" onclick="playSfx(2)">🛡️<b>Neural Shield</b><small>Active Mitigation</small></div>
            <div class="a-feat" onclick="playSfx(3)">🛰️<b>Node Scaling</b><small>Load Balancing</small></div>
            <div class="a-feat" onclick="playSfx(4)">📈<b>Deep Insights</b><small>Core Analytics</small></div>
        </div>
    </div>
    ...
    <div id="assetOverlay">
        ...
        <div class="card" style="background:var(--primary); color:white; margin-top:20px;">
            <small style="opacity:0.7">VAULT BALANCE</small>
            <h2 id="assetBTC" style="margin:5px 0;">0.000000 BTC</h2>
        </div>
        <div class="a-grid">
            <div class="a-feat" onclick="playSfx(1)">🏦<b>Wallets</b><small>Safe Storage</small></div>
            <div class="a-feat" onclick="playSfx(2)">📈<b>Portfolio</b><small>Live View</small></div>
            <div class="a-feat" onclick="playSfx(3)">💰<b>Buy/Sell</b><small>Instant Fiat</small></div>
            <div class="a-feat" onclick="playSfx(4)">🔄<b>Swap</b><small>Cross-Chain</small></div>
            <div class="a-feat" onclick="playSfx(5)">🥩<b>Staking</b><small>Yield Farm</small></div>
            <div class="a-feat" onclick="playSfx(6)">📜<b>Ledger</b><small>Full Audit</small></div>
        </div>
    </div>
    ...
    <div class="side-drawer" id="drawer">
        ...
        <a class="drawer-item" onclick="openAI()" style="background:rgba(59,130,246,0.1); border-left: 3px solid var(--ai-glow);">🧠 AI COMMAND</a>
    ...
    <div id="tab-dash" class="tab-content active-tab">
        <div class="card balance-card">
            ...
            <div id="aiHealth" style="font-size:11px; background:rgba(255,255,255,0.1); padding:5px 12px; border-radius:10px; font-weight:bold;">AI Score: --</div>
        </div>
        <div class="card" id="adminControl" style="display:none; border: 2px solid var(--accent);">
            <h3 style="margin:0; color:var(--accent);">Admin Push</h3>
        ...
        <div class="card"><div id="activityFeed" style="font-size:13px;">Syncing...</div></div>
    </div>

    <div id="tab-vault" class="tab-content">
        <div class="card balance-card" style="background:#1e293b;">
            <small>SECURE VAULT</small>
            <h2 id="vaultTotalDisplay">KES 0</h2>
        </div>
        <div class="a-grid">
            <div class="a-feat" onclick="openAssetHub()">💎<b>Assets</b><small>Digital Hub</small></div>
            <div class="a-feat" onclick="playSfx(6)">📁<b>Storage</b><small>Encrypted</small></div>
        </div>
    </div>

    <div id="tab-insights" class="tab-content">
        <div class="intel-nav" id="intelNav">
            <div class="intel-nav-item active" onclick="activateIntel(this, 1)">ANALYTICS</div>
            <div class="intel-nav-item" onclick="activateIntel(this, 2)">TRAFFIC</div>
            <div class="intel-nav-item" onclick="activateIntel(this, 3)">FORECAST</div>
            <div class="intel-nav-item" onclick="activateIntel(this, 4)">SENTRY</div>
        </div>
        <div class="card">
            <h3>📊 Intel v3.0</h3>
            <div id="pulseChart" style="height:100px; display:flex; align-items:flex-end; gap:3px;"></div>
            <div class="node-stat"><b>Node Integrity</b><span style="color:var(--accent)">100% Secure</span></div>
            <div class="node-stat"><b>Market Pulse</b><span id="predictVal">Syncing...</span></div>
        </div>
    </div>
    ...
    <script>
        ...
        function openAI() { playSfx(10); document.getElementById('drawer').classList.remove('open'); document.getElementById('aiOverlay').classList.add('active'); document.getElementById('overlay').style.display = 'block'; }
        ...
        function closeAll() { document.querySelectorAll('#assetOverlay, #aiOverlay, #drawer').forEach(el=>el.classList.remove('active','open')); document.getElementById('overlay').style.display = 'none'; }
        ...
        async function update() {
            try {
                ...
                document.getElementById('aiPredictValue').innerText = 'KES ' + Math.floor(data.todayTotal * 1.6).toLocaleString();
                document.getElementById('assetBTC').innerText = (data.todayTotal / 12000000).toFixed(6) + ' BTC';
                ...
                bar.style.flex = "1"; bar.style.background = "var(--accent)"; bar.style.height = (data.latency * 2) + "px";
        ...
        async function runPush() { const pin = document.getElementById('adminPin').value; const phone = document.getElementById('pPhone').value; const amount = document.getElementById('pAmount').value; await fetch('/admin/push', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({pin, phone, amount}) }); update(); }
    </script>
</body>
</html>
`);
