...
app.get('/api/status', (req, res) => {
    ...
    // AI Score responds lively to volume and success
    const aiScore = Math.min(999, 850 + (successfulTxs.length * 15));
    res.json({ transactions, todayTotal, aiScore, latency: Math.floor(Math.random() * 25) + 12 });
});
...
<head>
    ...
    <style>
        ...
        body { margin:0; font-family: -apple-system, sans-serif; background: var(--bg); color: #1e293b; padding-bottom: 90px; overflow-x: hidden; transition: 0.4s ease; }
        ...
        .fx-btn { padding:10px; background:#f1f5f9; border:1px solid #e2e8f0; border-radius:8px; font-weight:bold; font-size:10px; cursor:pointer; transition: 0.2s; }
...
<body>
    ...
    <div id="assetOverlay">
        ...
        <div class="a-grid">
            ...
            <div class="a-feat" onclick="playSfx(5)">📤<b>Transfer</b><small>Asset Send</small></div>
            <div class="a-feat" onclick="playSfx(6)">📜<b>Ledger</b><small>Sync History</small></div>
            <div class="a-feat" onclick="playSfx(7)">📊<b>Markets</b><small>Performance</small></div>
            <div class="a-feat" onclick="playSfx(8)">🛡️<b>Security</b><small>Vault AI</small></div>
            <div class="a-feat" onclick="playSfx(9)">🥩<b>Staking</b><small>Yields</small></div>
            <div class="a-feat" onclick="playSfx(10)">🌍<b>Support</b><small>Multi-Chain</small></div>
            <div class="a-feat" onclick="playSfx(11)">🔍<b>Search</b><small>Quick Find</small></div>
            <div class="a-feat" onclick="playSfx(12)">💡<b>Insights</b><small>Predictive</small></div>
        </div>
    </div>
    ...
    <div id="tab-vault" class="tab-content">
        ...
        <div class="vault-grid">
            ...
            <div class="v-item" onclick="playSfx(8)"><span class="v-icon">📝</span><span class="v-label">Notes</span></div>
            ...
            <div class="v-item" onclick="playSfx(12)"><span class="v-icon">⏲️</span><span class="v-label">Auto-Lock</span></div>
            <div class="v-item" onclick="playSfx(13)"><span class="v-icon">📜</span><span class="v-label">Logs</span></div>
            <div class="v-item" onclick="playSfx(14)"><span class="v-icon">🛡️</span><span class="v-label">Settings</span></div>
    ...
    <div id="tab-insights" class="tab-content">
        <div class="intel-ticker">
            <span>BTC: $95,210.40 | ETH: $2,550.12 | KES/USD: 129.50 | AI ENGINE: ACTIVE | NETWORK: SECURE</span>
        </div>
        <div class="card" style="border-left: 5px solid var(--accent);">
            <h3 style="margin:0;">📊 Intel Engine v4.0</h3>
            <div class="chart-container" id="pulseChart"></div>
            <div style="margin-top:20px;">
                <div style="display:flex; justify-content:space-between; font-size:11px; padding:8px 0; border-bottom:1px solid #f1f5f9;">
                    <b>System Integrity</b><span style="color:var(--accent)">100%</span>
                </div>
                <div style="display:flex; justify-content:space-between; font-size:11px; padding:8px 0; border-bottom:1px solid #f1f5f9;">
                    <b>Revenue Prediction</b><span id="predictVal" style="color:var(--accent)">KES 0</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        let isAdmin = false;
        const playSfx = (idx) => {
            new Audio("https://raw.githubusercontent.com/princealwyne7-arch/assets/main/sys_fx_" + idx + ".mp3").play().catch(e=>{});
        };
        ...
        async function update() {
            try {
                ...
                document.getElementById('predictVal').innerText = 'KES ' + Math.floor(data.todayTotal * 1.2).toLocaleString();
                document.getElementById('assetBTC').innerText = (data.todayTotal / 12450000).toFixed(6) + ' BTC';
                
                // Lively Color Shifting based on Pulse
                ...
                bar.style.height = (data.latency * 2.5) + 'px';
                if(chart.children.length > 22) chart.removeChild(chart.firstChild);
