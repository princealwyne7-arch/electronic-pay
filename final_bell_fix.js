...
// 1. STYLE: Add professional positioning for the bell
const bellStyle = `
    .header-tools { position: absolute; top: 25px; right: 25px; z-index: 1001; }
    .tool-icon { color: white; font-size: 24px; cursor: pointer; transition: 0.2s; }
`;
if (!c.includes('.header-tools')) {
    c = c.replace('</style>', bellStyle + '</style>');
}

// 2. HTML: Inject the Bell Icon into the Top Banner
const bellHTML = `
